...
import { FiUser, FiMail, FiPhone, FiCalendar, FiShield, FiLogOut, FiHeart, FiGrid, FiCompass, FiSettings, FiCheck, FiAlertCircle, FiLock } from 'react-icons/fi';

/**
 * UserDashboard Component
 * Dedicated customer portal page serving /customer/dashboard, /bookings, /profile, /wishlist, and /settings
 */
export default function UserDashboard({ activeTab = "dashboard" }) {
  ...
  // Dynamic Bookings & Wishlist State from LocalStorage
  ...
  useEffect(() => {
    ...
    // Load active reservations from local storage (defaulting to sample demo reservation if empty)
    try {
  ...
  const handleCancelBooking = (id) => {
    const updated = customerBookings.map(b => b.id === id ? { ...b, bookingStatus: "Cancelled", paymentStatus: "Voided" } : b);
                        ...
                        <div className="flex md:flex-col items-center md:items-end justify-between w-full md:w-auto gap-3 pt-3 md:pt-0 border-t md:border-t-0 border-stone-200 dark:border-stone-800">
                          ...
                          {bk.bookingStatus === "Confirmed" && (
                            <button 
                              ...
                              className="px-4 py-2 rounded-lg border border-rose-500/40 text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 text-[11px] font-semibold uppercase tracking-wider transition-colors"
                            >
                              Cancel Booking
                            </button>
                          )}
